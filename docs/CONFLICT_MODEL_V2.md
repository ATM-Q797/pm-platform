...
- 检测时跳过 `phase.phase_type in ("P8", "P9")` 的阶段（不参与冲突对生成——P8/P9 双兼容排除，见 §2.3 对齐段；f32717d 起三处全排除）
- **热力图/资源甘特同步排除**：P8/P9 阶段不占格、不计 peak、不显示（PHASE_TYPES_V2 §四，2026-08-28 起 P8 旧文"仍占格"作废——本文档已对齐）
...
**P8/P9 交付排除（PHASE_TYPES_V2 对齐，2026-08-28）**：交付语义重排为 P9，且 P8 变更为联调测试——冲突检测/热力图/资源甘特三处对 `P8` 与 `P9` **双兼容排除**（旧数据 P8 交付 + 新数据 P9 交付 + 新 P8 联调测试均不计入；f32717d 起热力/甘特已排除 P8，本处同步为 P8/P9）。
...
| # | 场景 | 预期 |
|---|------|------|
| 1 | P8/P9 阶段与 P5 重叠 | 不产生冲突对；热力图 P8/P9 不占格（PHASE_TYPES_V2 §四双兼容排除） |
...
- [ ] P8/P9 不参与热力计数、不参与冲突（双兼容排除——PHASE_TYPES_V2 §四）
