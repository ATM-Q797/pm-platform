...
- 跳过**所属项目状态为「搁置」**的阶段（`_active_phases` 或 detect_conflicts 内 join Project 判断）
...
1. **项目更新接口**：`status='搁置'` 合法；旧值 `'已搁置'` 是否也接受（兼容旧客户端/旧数据）？——**建议接受并归一化为「搁置」**（后端 update 时映射）
2. 阶段级「已搁置」确认**保持原名**不动？
