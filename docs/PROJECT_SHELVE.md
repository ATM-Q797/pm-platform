...
| # | 决策 | 结论 |
|---|------|------|
| 1 | 项目更新接口接受旧值「已搁置」吗？ | **接受并归一化为「搁置」**（后端 update 时映射；评审处置 #2）——测试 5 期望固定为「'搁置' 保存成功、'已搁置' 归一化保存成功、其他值 422」 |
| 2 | 阶段级「已搁置」改名吗？ | **不改**（与 §一 范围界定一致） |
| 3 | 现有资源甘特视图如何处理搁置项目？ | **排除**（§2.5，评审处置 #3——搁置不占资源，甘特/热力/负载口径统一） |
| 4 | 后端过滤键 | **双 key（「搁置」+ 旧值「已搁置」）**（评审处置 #7，与前端双 key 兼容一致，迁移前后行为稳定） |
