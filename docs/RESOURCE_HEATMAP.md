...
- **颜色分级**：
  ...
  > **消除粒度（用户 2026-08-28 决策）**：Drawer 内**每对冲突一行**独立消除（`conflict_details` 数组，含重叠窗口 `overlap_start ~ overlap_end` 影响范围提示）；消除一对只消该资源该对（该重叠覆盖的周 ⚠ 一并消失），**其余对保持冲突**待分别点击消除；甘特/热力图/报告三入口消除后**跨视图同步**（ResourcePage conflictVersion + `conflict-changed` 事件，Dashboard 抽屉联动）。
